...
const TYPE_CONFIG: Record<string, { color: string; label: string; icon: any }> = {
  ...
  challenge: { color: 'text-primary-400 border-l-primary-500 bg-primary-500/10', label: 'Challenges', icon: AlertTriangle },
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8 animate-fade-in">
      ...
      <div>
        ...
        <h2 className="text-3xl font-bold font-display text-surface-800 flex items-center gap-2">
          Memory & Insights
        </h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 select-none">
        {(Object.keys(TYPE_CONFIG) as Array<keyof typeof TYPE_CONFIG>).map((type) => {
          ...
          return (
            <div
              key={type}
              className={`glass-card border-l-[3px] border-y border-r border-surface-200/40 p-4 rounded-xl flex flex-col justify-between ${config.color}`}
            >
              ...
              <h4 className="text-2xl font-bold font-display text-surface-800 mt-3">{count}</h4>
            </div>
          )
        })}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-3">
            {insights.map((ins) => {
              ...
              const typeColor = ins.type === 'goal' ? 'bg-emerald-500/10 text-emerald-400' :
                                ...
                                ins.type === 'challenge' ? 'bg-primary-500/10 text-primary-400' :
                                'bg-purple-500/10 text-purple-400'
              return (
                <div
                  key={ins.id}
                  className="glass-card border border-surface-200/40 p-4.5 rounded-xl flex gap-4 items-start hover:border-surface-200/80 transition-all duration-200"
                >
                  <div className="w-10 h-10 rounded-xl bg-surface-50 border border-surface-200/40 flex items-center justify-center text-lg shrink-0 select-none">
                    {ins.icon}
                  </div>
                  <div className="flex-1 min-w-0 space-y-2">
                    <div className="flex items-start justify-between gap-4">
                      <p className="text-sm font-semibold text-surface-800 leading-relaxed font-body">
                        {ins.label}
                      </p>
                      {ins.explicit && (
                    ...
                    <div className="text-[10px] text-surface-400 font-medium select-none">
                      {ins.source} · {ins.date}
                    </div>
                    ...
                    <div className="flex items-center gap-3 select-none">
                      ...
                      <span className="text-[10px] font-semibold text-surface-500 min-w-10">
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg border border-surface-200/40 bg-surface-50/50 hover:border-red-500/30 text-surface-400 hover:text-red-400 shrink-0 transition-colors"
                  >
            ...
            {insights.length === 0 && (
              <div className="glass-card border border-surface-200/40 rounded-xl p-12 text-center text-surface-400 select-none">
                <Brain className="w-10 h-10 text-surface-300 mx-auto mb-2" />
                <h3 className="text-sm font-bold text-surface-600">Memory is empty</h3>
                <p className="text-xs text-surface-400 mt-1">No profile insights extracted yet.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-6 select-none">
          {/* NEETI knows you as */}
          <div className="glass-card border border-primary-500/15 bg-gradient-to-br from-primary-500/5 to-surface-50/10 p-5 rounded-2xl space-y-3">
            <div className="flex items-center gap-2 border-b border-surface-200/20 pb-3">
              <Brain className="w-5 h-5 text-primary-500" />
              <h3 className="text-sm font-bold text-surface-800 font-display">
                NEETI knows you as...
              </h3>
            </div>
          ...
          <div className="glass-card border border-surface-200/40 p-5 rounded-2xl space-y-4">
            ...
            <div className="space-y-4 text-xs">
              <div className="flex items-center justify-between gap-4">
                <span className="text-surface-600">Auto-extract insights from chat</span>
                <button
                  ...
                  className={`w-9 h-5 rounded-full relative transition-colors duration-200 shrink-0 ${
                    autoExtract ? 'bg-primary-500' : 'bg-surface-100'
                  }`}
                >
                  <span
              ...
              <div className="flex items-center justify-between gap-4">
                <span className="text-surface-600">Use memory to personalize responses</span>
                <button
                  onClick={() => handleToggle('neeti-privacy-usememory', !useMemory, setUseMemory)}
                  className={`w-9 h-5 rounded-full relative transition-colors duration-200 shrink-0 ${
                    useMemory ? 'bg-primary-500' : 'bg-surface-100'
                  }`}
                >
                  <span
                    className={`w-3.5 h-3.5 rounded-full bg-white absolute top-0.75 transition-all duration-200 ${
              ...
              <div className="flex items-center justify-between gap-4">
                <span className="text-surface-600">Share anonymous usage data</span>
                <button
                  onClick={() => handleToggle('neeti-privacy-sharedata', !shareData, setShareData)}
                  className={`w-9 h-5 rounded-full relative transition-colors duration-200 shrink-0 ${
                    shareData ? 'bg-primary-500' : 'bg-surface-100'
                  }`}
                >
                  <span
                    className={`w-3.5 h-3.5 rounded-full bg-white absolute top-0.75 transition-all duration-200 ${
              ...
              <div className="border-t border-surface-200/20 pt-4 flex gap-2">
                <button
                  onClick={clearMemory}
